package com.sportylight.security;
...
public class CustomUserDetailsService implements UserDetailsService {
	...
	@Override
	public UserDetails loadUserByUsername(String email)  // 로그인 시도 시 호출된다. username이 있으면 상세정보 리턴
			throws UsernameNotFoundException {				// id 즉, username이 없으면 예외처리
															// MemberVO는 security가 모르기 때문에 UserDetails로 변환해서 리턴해야함
		log.warn("Load User By Username: " + email);		// 로그 레벨을 warning으로 높였다.
		
		MemberVO vo = mapper.read2(email);
		if(vo == null) {
